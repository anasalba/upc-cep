package upc.edu.cep.flume.sources;
...
public class CEPKafkaSource extends AbstractPollableSource
    ...
    protected Status doProcess() throws EventDeliveryException {
        ...
        try {
            ...
            while (eventList.size() < batchUpperLimit &&
                ...
                //if(eventType.equals(null) || eventType=="")
                ...
                if (useAvroEventFormat) {
                    ...
                    if(thereIsType)
                    {
                        ...
                        {
                            String eventString = new String(eventBody);
                            Map map = gson.fromJson(eventString,HashMap.class);
                            GenericRecord avroRecord = new GenericData.Record(schema);
                            for (String att : attributeNames)
                            {
                                avroRecord.put(att,map.get(att));
                            }
                            eventBody = datumToByteArray(schema,avroRecord);
                        }
                    }
                }
    ...
    protected void doConfigure(Context context) throws FlumeException {
        ...
        eventType = context.getString(CustomTypeKafkaSourceConstants.EVENT_TYPE);
        if(eventType != null && !eventType.isEmpty()) {
            thereIsType = true;
        }
        eventName = context.getString(CustomTypeKafkaSourceConstants.EVENT_NAME);
        System.out.println("++++++++++++++++++++++++"+eventName);
        String eventAttributes = context.getString(CustomTypeKafkaSourceConstants.EVENT_ATTRIBUTES);
        if(eventAttributes != null && !eventAttributes.isEmpty()) {
            String[] atts = eventAttributes.trim().split(" ");
            attributes = new HashMap<>();
            for (String attribute : atts) {
                System.out.println("++++++++++++++++++++++++"+attribute);
                attributes.put(attribute, context.getString(attribute + "." + CustomTypeKafkaSourceConstants.ATTRIBUTE_TYPE));
